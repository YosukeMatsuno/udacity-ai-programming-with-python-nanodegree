...
def get_pet_labels(image_dir):
    ...
    for index in range(len(in_files)):
       ...
       if in_files[index][0] != ".":
           ...
           num_digit = lambda words:len([word for word in words if word.isdigit()])

           tmp_label = ""
           for words in pet_label:
               if num_digit(words) == 0:
                   if tmp_label == "":
                       tmp_label += words
                   else:
                       tmp_label += " " + words
           ...
           #          accessed by in_files[index]. Be certain to place the
           ...
           if in_files[index] not in results_dict:
              results_dict[in_files[index]] = [pet_label]
           else:
